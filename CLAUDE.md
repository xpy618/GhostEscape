...
鼠标状态不在 event 中获取：`Game::update()` 每帧通过 `SDL_GetMouseState()` 获取鼠标位置和按键，存入 `mouse_pos_` / `mouse_button_`，各对象在 `update()` 中通过 `Game::getInstance().getMousePosition()` 读取。

### 主循环

`Game::run()` (Game::getInstance().cpp) — 固定时间步长（60 FPS）的主循环：`handleEvent()` → `update(dt)` → `render()`。`dt` 为帧间隔（秒）。窗口使用 `SDL_LOGICAL_PRESENTATION_LETTERBOX` 模式保持逻辑分辨率。
...
- `Game::renderTexture()` (Game::getInstance().cpp:137) 传入了 `&texture.src_rect` 而非 `&src_rect`，导致纹理 mask/percentage 在源矩形上不生效。生命条/法力条通过 `Sprite::percentage_` 控制显示比例时，纹理会被压缩而非裁切。
